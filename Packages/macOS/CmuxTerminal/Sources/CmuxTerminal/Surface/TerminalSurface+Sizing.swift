...
extension TerminalSurface {
    ...
    public func updateSize(
        ...
        #if DEBUG
        Self.sizeLog("updateSize-call surface=\(id.uuidString.prefix(8)) size=\(wpx)x\(hpx) prev=\(lastPixelWidth)x\(lastPixelHeight) changed=\((force || scaleChanged || sizeChanged) ? 1 : 0)")
        #endif
        ...
        // that case.
        let deferScaleUntilResized = (force || scaleChanged) && (force || sizeChanged) && (xScale > lastXScale || yScale > lastYScale)
        if (force || scaleChanged) && !deferScaleUntilResized {
        ...
        if force || sizeChanged {
            ...
            let shouldApplySizeChange = force || Self.shouldApplySurfacePixelSizeChange(
