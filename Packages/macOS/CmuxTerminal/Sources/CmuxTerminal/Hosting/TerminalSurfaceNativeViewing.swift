...
public protocol TerminalSurfaceNativeViewing: NSView, TerminalSurfaceHosting {
    ...
    @discardableResult
    func forceRefreshSurface(forceGeometry: Bool) -> Bool
}

public extension TerminalSurfaceNativeViewing {
    /// Forces a synchronous surface size/draw refresh without rewriting unchanged geometry.
    @discardableResult
    func forceRefreshSurface() -> Bool {
        forceRefreshSurface(forceGeometry: false)
    }
}
